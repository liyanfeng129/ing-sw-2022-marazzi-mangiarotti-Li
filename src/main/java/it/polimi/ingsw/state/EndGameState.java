...
public class EndGameState extends State implements Serializable {
    ...
    public Command generateCommand() throws EriantysExceptions {
        ...
        //compute winner if a player finished his tower
        ...
        //if no one finished the tower, compute winner based on how many tower they used
        if (winners.size() == 0) {
            ...
            if (winners.size() >1) {
                ...
                //if number of tower used is even, compute winner based on which player has more professor
                for (int i = 0; i < winners.size(); i++) {
